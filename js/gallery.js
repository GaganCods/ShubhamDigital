...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Close popup when clicking anywhere on the overlay (outside the image)
    galleryPopup.addEventListener('click', function(e) {
        // Check if the click was directly on the popup background (not on the image or navigation buttons)
        if (e.target === galleryPopup || e.target.classList.contains('popup-content')) {
            galleryPopup.style.display = 'none';
        }
    });

    // Prevent clicks on the image and buttons from closing the popup
